...
def test_descubrir_multiples_reglas_mismo_hecho_max():
    ...
    hechos_lactosa = [t for t in resultados if t.terminos() == ["pizza", "contiene", "lactosa"]]
...
def test_descubrir_cadena_inferencias():
    ...
    assert any(t.terminos() == ["pizza", "contiene", "gluten"] and t.confianza == 0.8 for t in resultados)
    
    # Inferencia 2: pizza apto_celiaco no [MIN(1.0, 1.0, 0.8) = 0.8]
    assert any(t.terminos() == ["pizza", "apto_celiaco", "no"] and t.confianza == 0.8 for t in resultados)
