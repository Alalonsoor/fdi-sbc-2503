...
def test_unify_literales():
    ...
    # Iguales
    r = unify(
        Tripleta("tomate", "tipo", "verdura"), Tripleta("tomate", "tipo", "verdura")
    )
    assert len(r) == 1

    # Diferentes
    r = unify(
        Tripleta("tomate", "tipo", "verdura"), Tripleta("lechuga", "tipo", "verdura")
    )
    assert len(r) == 0
...
def test_unify_con_variables():
    ...
    # Variable en sujeto
    r = unify(Tripleta("X", "tipo", "verdura"), Tripleta("tomate", "tipo", "verdura"))
    assert len(r) == 1
    assert r[0].get("X") == "tomate"

    # Variables en ambos lados
    r = unify(Tripleta("X", "tipo", "Y"), Tripleta("tomate", "tipo", "verdura"))
    assert len(r) == 1
    assert r[0].get("X") == "tomate"
    assert r[0].get("Y") == "verdura"

    # Dos variables que unifican entre sí
    r = unify(Tripleta("X", "tipo", "verdura"), Tripleta("Y", "tipo", "verdura"))
    assert len(r) == 1
...
def test_unify_con_sustitucion_previa():
    ...
    r = unify(
        Tripleta("X", "tipo", "verdura"), Tripleta("tomate", "tipo", "verdura"), ss
    )
    assert len(r) == 1
